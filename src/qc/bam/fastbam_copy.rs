...
struct FastBamAnalysis {
    ...
    a_content_per_position: Vec<f64>,
    c_content_per_position: Vec<f64>,
    t_content_per_position: Vec<f64>,
    g_content_per_position: Vec<f64>,
    n_content_per_position: Vec<f64>,
    ...
    num_mapped_bases_per_position: Vec<usize>,
    coverage_per_position: Vec<i64>,
    ...
    mapping_quality_per_position: Vec<i64>,
    ...
    // insert size
    insert_size_per_position: Vec<f64>,
    num_reads_with_correct_insert_size_per_position: Vec<usize>,
...
impl FastBamAnalysis {
    ...
    pub fn new(_bam_file: String, qualimap_config: &QualimapConfig) -> Self {
        ...
        Self {
            ...
            a_content_per_position: vec![],
            c_content_per_position: vec![],
            t_content_per_position: vec![],
            g_content_per_position: vec![],
            n_content_per_position: vec![],
            ...
            // insert size
            num_mapped_bases_per_position: vec![],
            insert_size_per_position: vec![],
            num_reads_with_correct_insert_size_per_position: vec![],
    ...
    pub fn process_sequence(&mut self, record: &Record) {
        ...
        if read_is_valid {
            ...
            if record.is_proper_pair() && insert_size > 0 {
                ...
                self.num_reads_with_correct_insert_size_per_position
                    [absolute_position as usize - 1] += 1;
                self.insert_size_per_position[absolute_position as usize - 1] +=
                    insert_size.abs() as f64;
            }
            ...
            for j in absolute_position..=read_absolute_end {
                ...
                self.num_mapped_bases_per_position[j as usize - 1] += 1;
                ...
                if nucleotide != '-' {
                    ...
                    if mapping_quality != 0 {
                        self.mapping_quality_per_position[j as usize - 1] += mapping_quality as i64;
                    }
                    // coverage
                    self.coverage_per_position[j as usize - 1] += 1;

                    // ATCG content
                    match nucleotide {
                        'A' => {
                            self.number_of_as += 1;
                            self.a_content_per_position[j as usize - 1] += 1.0;
                        }
                        'C' => {
                            self.number_of_cs += 1;
                            self.c_content_per_position[j as usize - 1] += 1.0;
                        }
                        'T' => {
                            self.number_of_ts += 1;
                            self.t_content_per_position[j as usize - 1] += 1.0;
                        }
                        'G' => {
                            self.number_of_gs += 1;
                            self.g_content_per_position[j as usize - 1] += 1.0;
                        }
                        'N' => {
                            self.number_of_ns += 1;
                            self.n_content_per_position[j as usize - 1] += 1.0;
                        }
                        _ => {}
                    }
                }
    ...
    fn analyze_intervals(&mut self, interval: (usize, usize)) {
        ...
        for (index, result) in bam_reader.records().skip(interval.0).enumerate() {
    ...
    fn load_and_init(&mut self) {
        ...
        // set total records
        self.total_num_of_records = bam_reader.records().count();
        ...
        // allocate
        self.coverage_per_position = vec![0; self.reference_size as usize];
        self.mapping_quality_per_position = vec![0; self.reference_size as usize];
        self.a_content_per_position = vec![0.0; self.reference_size as usize];
        self.c_content_per_position = vec![0.0; self.reference_size as usize];
        self.t_content_per_position = vec![0.0; self.reference_size as usize];
        self.g_content_per_position = vec![0.0; self.reference_size as usize];
        self.n_content_per_position = vec![0.0; self.reference_size as usize];
        self.insert_size_per_position = vec![0.0; self.reference_size as usize];
        self.num_reads_with_correct_insert_size_per_position =
            vec![0; self.reference_size as usize];
        self.num_mapped_bases_per_position = vec![0; self.reference_size as usize];
    }
    ...
    pub fn merge(&mut self, other: &Self) {
        for i in 0..self.reference_size as usize {
            self.coverage_per_position[i] += other.coverage_per_position[i];
            self.mapping_quality_per_position[i] += other.mapping_quality_per_position[i];
            self.a_content_per_position[i] += other.a_content_per_position[i];
            self.t_content_per_position[i] += other.t_content_per_position[i];
            self.c_content_per_position[i] += other.c_content_per_position[i];
            self.g_content_per_position[i] += other.g_content_per_position[i];
            self.n_content_per_position[i] += other.n_content_per_position[i];
            self.insert_size_per_position[i] += other.insert_size_per_position[i];
            self.num_reads_with_correct_insert_size_per_position[i] +=
                other.num_reads_with_correct_insert_size_per_position[i];
            self.num_mapped_bases_per_position[i] += other.num_mapped_bases_per_position[i];
        }
    ...
    fn update_coverage_histogram(&mut self) {
        for i in 0..self.coverage_per_position.len() {
            // todo !() region analyze
            // if window.selected_regions_available_flag && !window.get_region().get(i).unwrap() {
            //     continue;
            // }

            let key = self.coverage_per_position[i] as i64;
    ...
    fn update_mapping_histogram(&mut self) {
        for i in 0..self.mapping_quality_per_position.len() {
            // todo !() region analyze
            // if window.selected_regions_available_flag && !window.get_region().get(i).unwrap() {
            //     continue;
            // }
            let quality = self.mapping_quality_per_position[i] as i64;
            if quality == -1 {
                continue;
            }
            ...
            if quality < Self::CACHE_SIZE as i64 {
                ...
                let mut value = self.mapping_quality_histogram_map.get(&quality).unwrap();
                let update_value = *value + 1;
    ...
    pub fn finish_metrics_across_reference(&mut self) {
        ...
        for i in 0..window_num {
            let window_start = self.window_starts[i];
            let window_end = self.window_ends[i];
            let effective_window_length = window_end - window_start + 1;

            // sum of mapped base content in a window
            let mut sum_of_as = 0.0;
            let mut sum_of_ts = 0.0;
            let mut sum_of_cs = 0.0;
            let mut sum_of_gs = 0.0;
            let mut sum_of_ns = 0.0;

            // sum of coverage in a window
            let mut sum_of_mapped_base = 0;
            let mut sum_coverage_squared = 0;
            let mut sum_coverage = 0;

            // sum of mapping in a window
            let mut sum_mapping_quality = 0;

            // sum of insert size in a window
            let mut sum_insert_sizes = 0.0;
            let mut sum_reads_with_correct_insert_sizes = 0;

            for pos in window_start - 1..=window_end - 1 {
                // acum num of mapped base
                sum_of_mapped_base += self.num_mapped_bases_per_position[pos];

                // acum mapping quality
                sum_mapping_quality += self.mapping_quality_per_position[pos];

                // acum insert size
                sum_insert_sizes += self.insert_size_per_position[pos];
                sum_reads_with_correct_insert_sizes +=
                    self.num_reads_with_correct_insert_size_per_position[pos];

                // acum base
                sum_of_as += self.a_content_per_position[pos];
                sum_of_ts += self.t_content_per_position[pos];
                sum_of_cs += self.c_content_per_position[pos];
                sum_of_gs += self.g_content_per_position[pos];
                sum_of_ns += self.n_content_per_position[pos];

                let coverage_at_postion = self.coverage_per_position[pos];
                if coverage_at_postion > 0 {
                    // reduce quality
                    self.mapping_quality_per_position[pos] =
                        self.mapping_quality_per_position[pos] / coverage_at_postion as i64;

                    // acum coverage
                    sum_coverage_squared += coverage_at_postion * coverage_at_postion;
                    sum_coverage += coverage_at_postion;
                } else {
                    //  make it invalid for histogram
                    self.mapping_quality_per_position[pos] = -1;
                }
            }

            // push average coverage results
            ...
            let mean_coverage = sum_of_mapped_base as f64 / effective_window_length as f64;
            self.coverage_across_reference[i] = mean_coverage;

            let floor_mean_coverage =
            ...
            let std_coverage = (sum_coverage_squared as f64 / effective_window_length as f64
                - floor_mean_coverage as f64 * floor_mean_coverage as f64)
                .sqrt();
            self.std_coverage_across_reference[i] = std_coverage;

            // push average insert size results
            if sum_reads_with_correct_insert_sizes > 0 {
                self.insert_size_across_reference[i] =
                    sum_insert_sizes / sum_reads_with_correct_insert_sizes as f64;
            }

            if mean_coverage > 0.0 {
                // push average mapping quality results
                let mean_mapping_quality = sum_mapping_quality as f64 / sum_of_mapped_base as f64;
                self.mapping_quality_across_reference[i] = mean_mapping_quality;

                // push average base content results
                let mean_a_content = sum_of_as / mean_coverage;
                let mean_t_content = sum_of_ts / mean_coverage;
                let mean_c_content = sum_of_cs / mean_coverage;
                let mean_g_content = sum_of_gs / mean_coverage;
                let mean_n_content = sum_of_ns / mean_coverage;
    ...
    pub fn run(&mut self, bam_file: &str, qualimap_config: &QualimapConfig) {
        ...
        for i in 1..=fast_bam_analysis.thread_num {
            ...
            handles.push(thread::spawn(move || {
                ...
                fast_bam_analysis_instance.load_and_init();
        ...
        println!("Time to merge: {:?}", Instant::now() - start_time_to_merge);

        let start_time_to_finish = Instant::now();
        fast_bam_analysis.finish();
        println!(
            "Time to finish: {:?}",
            Instant::now() - start_time_to_finish
        );
    }
